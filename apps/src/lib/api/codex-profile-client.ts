...
export function normalizeCodexProfileStatus(payload: unknown): CodexProfileStatus {
  ...
  return {
    ...
    actualAccountId: toNullableString(
      source.actualAccountId ?? source.actual_account_id,
    ),
    identityConsistent: asBoolean(
      source.identityConsistent ?? source.identity_consistent,
    ),
