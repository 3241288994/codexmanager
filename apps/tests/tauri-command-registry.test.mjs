...
test("桌面壳不注册服务端已废弃的历史 RPC", async () => {
  const registrySource = await readSource("src-tauri/src/commands/registry.rs");

  assert.doesNotMatch(registrySource, /service_plugin_/);
  assert.doesNotMatch(registrySource, /service_account_(?:import|export|warmup)/);
});
