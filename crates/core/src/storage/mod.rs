...
impl Storage {
    ...
    fn ensure_column(&self, table: &str, column: &str, column_type: &str) -> Result<()> {
        ...
        match self.conn.execute(&sql, []) {
            Ok(_) => Ok(()),
            // Another process may have added the column after the check above while
            // initializing the same fresh SQLite database. Verify that race and
            // continue with the resulting schema.
            Err(err)
                if Self::is_schema_conflict_error(&err) && self.has_column(table, column)? =>
            {
                Ok(())
            }
            Err(err) => Err(err),
