use chrono::TimeZone;
use codexmanager_core::auth::{extract_token_exp, DEFAULT_CLIENT_ID};
...
#[derive(Debug, Clone)]
pub(crate) struct ProfileCredentialSync {
...
pub(crate) fn active_direct_account_id() -> Option<String> {
    let status = get_status(None).ok()?;
    if !matches!(status.mode, CodexProfileMode::DirectAccount) || !status.identity_consistent {
        return None;
    }
    status.selected_account_id
}
...
pub(crate) fn apply_direct_account(
    ...
    let previous_auth = read_optional(&profile_dir.join(AUTH_FILE))?;
    let previous_config = read_optional(&profile_dir.join(CONFIG_FILE))?;
    let managed_paths = managed_profile_paths(&profile_dir)?;
    let previous_marker = read_optional(&managed_paths.marker_path)?;
    ...
    let switch_result = write_profile_files(
        &profile_dir,
        &auth_json,
        &config_toml,
        ManagedState {
    ...
    .and_then(|_| persist_codex_home(&profile_dir));
    if let Err(write_error) = switch_result {
        let rollback_error = restore_switch_snapshot(
            &profile_dir,
            previous_auth.as_deref(),
            previous_config.as_deref(),
            &managed_paths.marker_path,
        ...
        return Err(match rollback_error {
            Some(rollback_error) => format!(
                "account switch failed: {write_error}; rollback also failed: {rollback_error}"
            ),
            None => format!("account switch failed and was rolled back: {write_error}"),
        });
    }
    // Direct OpenAI account switches do not change the model provider.  Keep
    // credential switching independent from history maintenance: scanning a
    // large Codex session archive here made a simple account switch read (and
    // occasionally rewrite) many gigabytes of unrelated conversation data.
    let status = status_for_profile(&profile_dir)?;
    if !status.identity_consistent {
        let rollback_error = restore_switch_snapshot(
            &profile_dir,
            previous_auth.as_deref(),
            previous_config.as_deref(),
        ...
        return Err(match rollback_error {
            Some(rollback_error) => format!(
                "account switch verification failed; rollback also failed: {rollback_error}"
            ),
            None => "account switch verification failed and was rolled back".to_string(),
        });
    }
    Ok(status)
}

fn restore_switch_snapshot(
    profile_dir: &Path,
    ...
    for result in [
        restore_optional_file(&profile_dir.join(AUTH_FILE), auth_json),
        restore_optional_file(&profile_dir.join(CONFIG_FILE), config_toml),
        restore_optional_file(marker_path, marker_json),
    ] {
    ...
    let state_result = match state {
        Some(state) => save_state(state),
        None => crate::app_settings::save_persisted_app_setting(APP_SETTING_STATE_KEY, None),
    };
...
fn status_for_profile(profile_dir: &Path) -> Result<CodexProfileStatus, String> {
    ...
    let actual_account_id = read_actual_account_id(&auth_path);
    let selected_account_id = state.as_ref().and_then(|state| state.account_id.clone());
    let identity_consistent = match (selected_account_id.as_deref(), actual_account_id.as_deref()) {
        (Some(selected), Some(actual)) => account_ids_match(selected, actual),
        (None, None) => true,
        _ => false,
    };
...
fn read_actual_account_id(auth_path: &Path) -> Option<String> {
...
/// Synchronize credentials that Codex CLI has refreshed in `auth.json` back to
/// the matching managed account.  The profile is authoritative only when its
/// recorded account and token account IDs agree; this prevents credentials
/// from ever crossing account boundaries.
pub(crate) fn sync_active_profile_credentials_for_account(
    account_id: &str,
) -> Result<Option<ProfileCredentialSync>, String> {
    let profile_dir = resolve_profile_dir(None)?;
    let status = status_for_profile(&profile_dir)?;
    if !matches!(status.mode, CodexProfileMode::DirectAccount)
        || status.selected_account_id.as_deref() != Some(account_id)
    {
        return Ok(None);
    }
    if !status.identity_consistent {
        return Err(
            "current Codex profile identity does not match the managed account".to_string(),
        );
    }
    ...
    let raw = fs::read_to_string(&auth_path).map_err(|err| {
        format!(
            "read current Codex auth failed ({}): {err}",
            auth_path.display()
        )
    })?;
    let value: serde_json::Value = serde_json::from_str(&raw)
        .map_err(|err| format!("parse current Codex auth failed: {err}"))?;
    let tokens = value
        .get("tokens")
        .and_then(serde_json::Value::as_object)
        .ok_or_else(|| "current Codex auth has no token object".to_string())?;
    let actual_account_id = tokens
        .get("account_id")
        .and_then(serde_json::Value::as_str)
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "current Codex auth has no account_id".to_string())?;
    if !account_ids_match(account_id, actual_account_id) {
        return Err("current Codex auth belongs to a different account".to_string());
    }

    let storage = open_storage()?;
    let existing = storage
        .find_token_by_account_id(account_id)
        .map_err(|err| format!("read managed account token failed: {err}"))?
        .ok_or_else(|| "managed account token not found".to_string())?;
    let access_token = token_field(tokens, "access_token")
        .ok_or_else(|| "current Codex auth has no access_token".to_string())?;
    let id_token = token_field(tokens, "id_token").unwrap_or_else(|| existing.id_token.clone());
    let refresh_token =
        token_field(tokens, "refresh_token").unwrap_or_else(|| existing.refresh_token.clone());
    let profile_refreshed_at = value
        .get("last_refresh")
        .and_then(serde_json::Value::as_str)
    ...
    let candidate = Token {
        ...
        access_token,
        refresh_token,
        api_key_access_token: existing.api_key_access_token.clone(),
        last_refresh: profile_refreshed_at,
    };
    let changed = token_credentials_differ(&existing, &candidate)
        && profile_token_is_at_least_as_fresh(&candidate, &existing);
    if changed {
        storage
            .insert_token(&candidate)
            .map_err(|err| format!("store current Codex credentials failed: {err}"))?;
        let access_exp = extract_token_exp(&candidate.access_token);
        let refresh_exp = extract_token_exp(&candidate.refresh_token);
        let ahead = crate::usage_token_refresh::token_refresh_ahead_secs();
        let next_refresh_at = match (access_exp, refresh_exp) {
            (Some(access), Some(refresh)) => Some(access.min(refresh).saturating_sub(ahead)),
            (Some(access), None) => Some(access.saturating_sub(ahead)),
            (None, Some(refresh)) => Some(refresh.saturating_sub(ahead)),
            (None, None) => None,
        };
        let _ = storage.update_token_refresh_schedule(account_id, access_exp, next_refresh_at);
        log::info!("synchronized current Codex profile credentials: account_id={account_id}");
    }

    Ok(Some(ProfileCredentialSync {
        account_id: account_id.to_string(),
        changed,
    }))
}

fn token_field(tokens: &serde_json::Map<String, serde_json::Value>, key: &str) -> Option<String> {
    tokens
        .get(key)
        .and_then(serde_json::Value::as_str)
        .map(str::trim)
...
fn token_credentials_differ(left: &Token, right: &Token) -> bool {
    left.access_token != right.access_token
        || left.refresh_token != right.refresh_token
        || left.id_token != right.id_token
}

fn profile_token_is_at_least_as_fresh(profile: &Token, stored: &Token) -> bool {
    match (
        extract_token_exp(&profile.access_token),
        extract_token_exp(&stored.access_token),
    ) {
        (Some(profile_exp), Some(stored_exp)) => {
            profile_exp > stored_exp
                || (profile_exp == stored_exp && profile.last_refresh >= stored.last_refresh)
        }
        (Some(_), None) => true,
        (None, Some(stored_exp)) => {
            stored_exp <= now_ts() && profile.last_refresh >= stored.last_refresh
        }
        (None, None) => profile.last_refresh >= stored.last_refresh,
    }
}

fn account_ids_match(managed_id: &str, actual_id: &str) -> bool {
    managed_id == actual_id || managed_id.contains(&format!("cgpt={actual_id}"))
}
...
mod tests {
    ...
    fn test_token(access_token: String, last_refresh: i64) -> Token {
        Token {
    ...
    #[test]
    fn profile_token_sync_never_replaces_a_newer_access_token() {
        let stored = test_token(jwt_with_exp(200), 100);
        let older_profile = test_token(jwt_with_exp(150), 300);
        let newer_profile = test_token(jwt_with_exp(250), 200);

        assert!(!profile_token_is_at_least_as_fresh(&older_profile, &stored));
        assert!(profile_token_is_at_least_as_fresh(&newer_profile, &stored));
    }
...
"#;
    ...
    fn write_profile_files_uses_internal_marker() {
        ...
        #[cfg(unix)]
        {
            ...
            assert_eq!(
                fs::metadata(dir.join(AUTH_FILE))
            ...
            assert_eq!(
                fs::metadata(dir.join(CONFIG_FILE))
