...
const MEMBER_METHOD_ALLOWLIST: &[&str] = &[
    // Member methods must operate on the authenticated actor's own data. Server
    // accounts, Codex homes, session catalogs, aggregate provider credentials,
    // LabContext administration, gateway controls, quotas, and service settings
    // are intentionally administrator-only.
...
pub(crate) fn handle_request_with_actor(req: JsonRpcRequest, actor: RpcActor) -> JsonRpcMessage {
    if req.method == "initialize" {
        ...
        let result = InitializeResult {
            ...
            user_agent: format!(
                "codex_cli_rs/{}",
                crate::default_gateway_user_agent_version()
            ),
    ...
    if let Some(resp) = labcontext::try_handle(&req) {
        return JsonRpcMessage::Response(resp);
    }
    ...
    if let Some(resp) = session_catalog::try_handle(&req) {
        return JsonRpcMessage::Response(resp);
    }
