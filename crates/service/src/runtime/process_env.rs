...
pub(crate) fn read_rpc_token_from_env_or_file() -> Option<String> {
    ...
    let token = read_rpc_token_from_file(&path);
    if token.is_some() {
        harden_rpc_token_permissions(&path);
    }
    token
}
...
#[cfg(unix)]
fn harden_rpc_token_permissions(path: &Path) {
    use std::os::unix::fs::PermissionsExt;

    if let Err(err) = fs::set_permissions(path, fs::Permissions::from_mode(0o600)) {
        log::warn!(
            "set rpc token permissions failed: {} ({})",
            path.to_string_lossy(),
            err
        );
    }
...
#[cfg(not(unix))]
fn harden_rpc_token_permissions(_path: &Path) {}
