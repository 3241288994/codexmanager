...
pub(crate) struct ManualUsageRefreshResult {
    pub ok: bool,
    pub account_id: String,
    pub credentials_synced_from_profile: bool,
    pub availability_status: String,
}

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct UsageRefreshReportItem {
    pub account_id: String,
    pub ok: bool,
    pub credentials_synced_from_profile: bool,
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct UsageRefreshReport {
    pub items: Vec<UsageRefreshReportItem>,
...
pub(crate) fn refresh_tokens_before_expiry_for_all_accounts() -> Result<(), String> {
    ...
    // The active profile is owned by Codex CLI. Codex may rotate its refresh
    // token independently, so the background manager must not race it. Manual
    // usage refresh first reconciles auth.json and then uses that access token.
    if let Some(active_account_id) = crate::codex_profile::active_direct_account_id() {
        tokens.retain(|token| token.account_id != active_account_id);
    }
    if tokens.is_empty() {
        return Ok(());
    }
...
pub(crate) fn refresh_usage_for_account(
    account_id: &str,
) -> Result<ManualUsageRefreshResult, String> {
    let profile_sync =
        crate::codex_profile::sync_active_profile_credentials_for_account(account_id)?;
    let credentials_synced_from_profile = profile_sync
        .as_ref()
        .is_some_and(|sync| sync.changed && sync.account_id == account_id);
    // 刷新单个账号用量
    let storage = open_storage().ok_or_else(|| "storage unavailable".to_string())?;
    ...
    {
        Some(token) => token,
        None => return Err("account token not found".to_string()),
    };
    ...
    if storage
        ...
        .ok()
        .flatten()
        .is_some_and(|account| account.status.trim().eq_ignore_ascii_case("unavailable"))
    {
        crate::account_status::set_account_status(
            &storage,
            account_id,
            "active",
            "usage_refresh_succeeded",
        );
    }
    record_usage_refresh_metrics(true, started_at);
    notify_usage_refresh_completed("single", 1, 1);
    Ok(ManualUsageRefreshResult {
        ok: true,
        account_id: account_id.to_string(),
        credentials_synced_from_profile,
        availability_status: availability_status.as_code().to_string(),
    })
}

pub(crate) fn refresh_usage_for_all_accounts_report() -> Result<UsageRefreshReport, String> {
    let storage = open_storage().ok_or_else(|| "storage unavailable".to_string())?;
    ...
    for account_id in account_ids {
        match refresh_usage_for_account(&account_id) {
            Ok(result) => items.push(UsageRefreshReportItem {
                account_id,
                ok: true,
                credentials_synced_from_profile: result.credentials_synced_from_profile,
                availability_status: Some(result.availability_status),
                error: None,
            }),
            Err(error) => items.push(UsageRefreshReportItem {
                account_id,
                ok: false,
                credentials_synced_from_profile: false,
